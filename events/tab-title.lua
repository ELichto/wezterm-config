...
local function get_current_working_dir(tab)
   ...
   wezterm.log_info(HOME_DIR)
   wezterm.log_info(current_dir.file_path)
   return current_dir == HOME_DIR and '.'
      or string.gsub(current_dir.file_path, '.*/([^/]+)/$', '%1')
end
...
M.setup = function()
   wezterm.on('format-tab-title', function(tab, _tabs, _panes, _config, hover, max_width)
      ...
      local title_name = get_current_working_dir(tab)
      -- local title = _set_title(process_name, tab.active_pane.title, max_width, (is_admin and 8))
      local title = title_name:len() > 0 and title_name
         or _set_title(process_name, tab.active_pane.title, max_width, (is_admin and 8))

      wezterm.log_info('title')
      wezterm.log_info(title_name)
